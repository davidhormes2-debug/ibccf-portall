...
// ============================================================================
// Source-assertion: App.tsx triggers stage history pruning on every page load
//
// WHY THIS TEST EXISTS
// `pruneStageHistory` is exported from `lib/stageHistory.ts` and called at
// module scope in App.tsx so stale/oversized stage-history entries are trimmed
// on every fresh page load without any user interaction.
...
//   1. App.tsx imports `pruneStageHistory` from `@/lib/stageHistory`.
//   2. App.tsx calls `pruneStageHistory()` as a top-level
...
describe("App.tsx stage history prune source assertion", () => {
  it("imports pruneStageHistory from @/lib/stageHistory", () => {
    const source = fs.readFileSync(APP_TSX, "utf-8");

    const hasImport =
      /import\s*\{[^}]*pruneStageHistory[^}]*\}\s*from\s*['"]@\/lib\/stageHistory['"]/.test(
        source,
      );

    expect(
      hasImport,
      "App.tsx must import pruneStageHistory from '@/lib/stageHistory'. " +
        "The call at module scope depends on this import — do not remove it.",
    ).toBe(true);
  });

  it("calls pruneStageHistory() at module scope (top-level Program.body)", () => {
    const source = fs.readFileSync(APP_TSX, "utf-8");

    const ast = parse(source, { jsx: true, range: false, loc: false });

    const hasTopLevelCall = isTopLevelCall(ast.body, "pruneStageHistory");

    expect(
      hasTopLevelCall,
      "App.tsx must call pruneStageHistory() as a top-level statement " +
