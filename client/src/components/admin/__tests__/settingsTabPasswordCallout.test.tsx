...
//
// Integration guard: SettingsTab password callout fetch→state→render pipeline
//
// The `callout-escape-hatch-password` element in SettingsTab.tsx is rendered
...
// extraction of `weakAdminPasswordAllowed` / `isProduction` from the
...
//   - it renders the callout div only when weakAdminPasswordAllowed=true AND
//     isProduction=false
//
// Contracted behaviours:
//   (p-1) callout renders when endpoint returns weakAdminPasswordAllowed=true
//         and isProduction=false.
//   (p-2) callout does NOT render when isProduction=true (even if flag is true).
//   (p-3) callout does NOT render when weakAdminPasswordAllowed=false.
...
// ── harness ────────────────────────────────────────────────────────────────
//
// Replicates only the escapeHatchFlags fetch + state + password callout
...
function SettingsTabPasswordCalloutHarness({ authToken }: { authToken: string }) {
  ...
  return (
    <>
      {escapeHatchFlags &&
        ...
        escapeHatchFlags.weakAdminPasswordAllowed && (
          <div
            data-testid="callout-escape-hatch-password"
            role="note"
          >
            Password strength check bypassed
          </div>
        )}
    </>
  );
...
describe("SettingsTab — password callout fetch→state→render", () => {
  it("(p-1) renders the callout when endpoint returns weakAdminPasswordAllowed=true and isProduction=false", async () => {
    mockSecurityFlags({
      ...BASE_FLAGS,
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-test" />);
    });

    expect(
      screen.queryByTestId("callout-escape-hatch-password"),
    ).not.toBeNull();
  });

  it("(p-2) does NOT render the callout when isProduction=true (even if flag is true)", async () => {
    mockSecurityFlags({
      ...BASE_FLAGS,
      weakAdminPasswordAllowed: true,
      isProduction: true,
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-p2" />);
    });
  ...
  it("(p-3) does NOT render the callout when weakAdminPasswordAllowed=false", async () => {
    mockSecurityFlags({
      ...BASE_FLAGS,
      weakAdminPasswordAllowed: false,
      isProduction: false,
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-p3" />);
    });
    ...
    // Auth header must be sent even when the callout is suppressed by weakAdminPasswordAllowed=false
    expect(global.fetch as Mock).toHaveBeenCalledWith(
      "/api/admin/security-flags",
      expect.objectContaining({
        headers: expect.objectContaining({
          Authorization: "Bearer tok-p3",
        }),
      }),
    );
  });
  ...
  it("does NOT render the callout when isProduction is missing from the response", async () => {
    ...
    // even though weakAdminPasswordAllowed is present and true.
    (global.fetch as Mock).mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        weakAdminPasswordAllowed: true,
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-guard1" />);
    });
  ...
  it("does NOT render the callout when weakAdminPasswordAllowed is missing from the response", async () => {
    // Guard variant 2: weakAdminPasswordAllowed omitted — setEscapeHatchFlags
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-guard2" />);
    });
  ...
  it("does NOT render the callout when weakAdminUsernameAllowed is missing from the response", async () => {
    // Guard variant 3: weakAdminUsernameAllowed omitted — the four-field guard
    // must reject the response even though the password flag and isProduction
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-guard3" />);
    });
  ...
  it("does NOT render the callout when weakSessionSecretAllowed is missing from the response", async () => {
    ...
    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="tok-guard4" />);
    });
  ...
  it("sends the Authorization header with the provided token", async () => {
    mockSecurityFlags({ ...BASE_FLAGS, weakAdminPasswordAllowed: true });

    await act(async () => {
      render(<SettingsTabPasswordCalloutHarness authToken="my-admin-token" />);
    });
