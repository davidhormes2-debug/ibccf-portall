// E2E smoke: verifies that POST /api/visitors/typing enforces its
...
//   satisfaction rate-limit IP-isolation specs.
// • Requests 1–120 from a given IP reach the route handler (TYPING_RATE_MAX
//   = 120).  They may return 200 or a non-429 status (400 if validation
//   rejects, etc.).  We assert on !429 rather than 200 to avoid a dependency
//   on business-logic preconditions.  We reuse the same caseId so the
//   in-memory TYPING_INDICATORS_MAX cap (500 unique map keys) is never
//   approached — the map key stays constant across all requests.
...
function freshIp(): string {
  const n = ipSuffix++;
  return `10.213.${Math.floor(n / 256)}.${n % 256}`;
}

const TYPING_RATE_MAX = 120;

function typingBody(caseId: string) {
  return {
    caseId,
    sender: "user",
    isTyping: true,
...
test.describe("POST /api/visitors/typing — IP-isolated rate limit (live server)", () => {
  test.setTimeout(180_000);

  test("120 POSTs from the same IP pass the rate limiter; the 121st gets 429", async ({
    request,
  }) => {
    const ipA = freshIp();
    const caseId = `e2e-ty-rl-${ipA.replace(/\./g, "-")}`;

    for (let i = 0; i < TYPING_RATE_MAX; i++) {
      const res = await request.post("/api/visitors/typing", {
        headers: { "X-Forwarded-For": ipA },
        data: typingBody(caseId),
      });
      expect(
        res.status(),
    ...
    const blocked = await request.post("/api/visitors/typing", {
      headers: { "X-Forwarded-For": ipA },
      data: typingBody(caseId),
    });
    expect(
  ...
  test("a different IP is not blocked after another IP exhausts its quota", async ({
    ...
    const ipA = freshIp();
    const ipB = freshIp();
    const caseIdA = `e2e-ty-rl-${ipA.replace(/\./g, "-")}`;
    const caseIdB = `e2e-ty-rl-${ipB.replace(/\./g, "-")}`;

    for (let i = 0; i < TYPING_RATE_MAX; i++) {
      await request.post("/api/visitors/typing", {
        headers: { "X-Forwarded-For": ipA },
        data: typingBody(caseIdA),
      });
    }

    const blockedA = await request.post("/api/visitors/typing", {
      headers: { "X-Forwarded-For": ipA },
      data: typingBody(caseIdA),
    });
    expect(
    ...
    const allowedB = await request.post("/api/visitors/typing", {
      headers: { "X-Forwarded-For": ipB },
      data: typingBody(caseIdB),
    });
    expect(
      allowedB.status(),
