...
//
// Structural guard: every describe("Task #148 — *") block in
// adminMutationTransactionsTask148.test.ts MUST contain:
...
const SRC = readFileSync(
  resolve(__dirname, "adminMutationTransactionsTask148.test.ts"),
  "utf-8",
);
...
/**
 * Find every `describe("Task #148 — *", ...)` call in `src` and return the
 * name + body text of each block.
 */
function extractTask148DescribeBlocks(
  src: string,
): { name: string; body: string }[] {
  const pattern = /describe\("(Task #148 \u2014 [^"]+)"/g;
...
describe("Structural guard — adminMutationTransactionsTask148.test.ts blocks", () => {
  const blocks = extractTask148DescribeBlocks(SRC);

  it("finds at least one Task #148 describe block (parser sanity check)", () => {
