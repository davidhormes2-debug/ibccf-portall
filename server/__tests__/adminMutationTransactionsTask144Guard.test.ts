...
//
// Structural guard: every describe("Task #144 — *") block in
// adminMutationTransactionsTask144.test.ts MUST contain:
...
const SRC = readFileSync(
  resolve(__dirname, "adminMutationTransactionsTask144.test.ts"),
  "utf-8",
);
...
/**
 * Find every `describe("Task #144 — *", ...)` call in `src` and return the
 * name + body text of each block.
 */
function extractTask144DescribeBlocks(
  src: string,
): { name: string; body: string }[] {
  const pattern = /describe\("(Task #144 \u2014 [^"]+)"/g;
...
describe("Structural guard — adminMutationTransactionsTask144.test.ts blocks", () => {
  const blocks = extractTask144DescribeBlocks(SRC);

  it("finds at least one Task #144 describe block (parser sanity check)", () => {
    ...
    it(`"${block.name}" has an it() whose title includes "rolls back"`, () => {
      ...
      expect(
        ...
        [
          ...
          ``,
          `Add a test like:`,
          `  it("rolls back <operation> when the audit write fails", async () => {`,
          `    auditShouldThrow = true;`,
          `    const res = await request(app()).post("...").set("Authorization", "Bearer t");`,
    ...
    it(`"${block.name}" has an it() whose title includes "commits"`, () => {
      ...
      expect(
        ...
        [
          ...
          ``,
          `Add a test like:`,
          `  it("commits <operation> and audit on happy path", async () => {`,
          `    const res = await request(app()).post("...").set("Authorization", "Bearer t");`,
