...
// Task #811 — API-level coverage for the on-demand wallet-connect alert
// marker cleanup trigger: POST
// /api/admin/wallet-connect-alert-marker-cleanup/run.
//
// Verifies:
//   1. The route is guarded by admin bearer auth (401 without a valid token).
//   2. The happy path returns the sweep result shape { deleted, scanned,
//      skipped } and writes the manual-trigger audit row inside the wrapping
//      transaction.
//   3. The sweep + trigger audit are wrapped in storage.runInTransaction, so
//      an audit-write failure rolls back (the route surfaces a 500).

const auditLogs: any[] = [];
let auditShouldThrow = false;

// The sweep result the (mocked) service returns for the happy path.
let sweepResult = { deleted: 3, scanned: 7, skipped: false };
const cleanupMock = vi.fn(async () => sweepResult);

vi.mock("../services/walletConnectAlert", () => ({
  cleanupOrphanedWalletConnectAlertMarkers: cleanupMock,
}));
...
const ENDPOINT = "/api/admin/wallet-connect-alert-marker-cleanup/run";

beforeEach(() => {
  auditLogs.length = 0;
  auditShouldThrow = false;
  sweepResult = { deleted: 3, scanned: 7, skipped: false };
  cleanupMock.mockClear();
});

describe("Task #811 — POST wallet-connect-alert-marker-cleanup/run auth", () => {
...
describe("Task #811 — POST wallet-connect-alert-marker-cleanup/run happy path", () => {
  it("runs the sweep and returns { deleted, scanned, skipped }", async () => {
    const res = await request(buildApp())
      .post(ENDPOINT)
      .set("Authorization", "Bearer good-token");

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ deleted: 3, scanned: 7, skipped: false });
    expect(cleanupMock).toHaveBeenCalledTimes(1);

    // The manual-trigger audit row is written with the sweep summary.
    const trigger = auditLogs.find(
      (a) => a.action === "wallet_connect_alert_marker_cleanup_run",
    );
  ...
  it("passes the skipped result straight through when a sweep was in progress", async () => {
    sweepResult = { deleted: 0, scanned: 0, skipped: true };
    const res = await request(buildApp())
      .post(ENDPOINT)
      .set("Authorization", "Bearer good-token");

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ deleted: 0, scanned: 0, skipped: true });
  });
});

describe("Task #811 — POST wallet-connect-alert-marker-cleanup/run failure", () => {
