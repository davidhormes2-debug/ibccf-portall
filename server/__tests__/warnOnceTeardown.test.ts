...
// ============================================================================
// Source-assertion: warnOnce dedup reset in test teardown
//
...
//
// SCOPE
// The scan covers all `*.test.ts` files under `server/` recursively, matching
// the Vitest `include` glob from vitest.config.ts (`server/**/*.test.ts`).
// ============================================================================
...
describe("warnOnce dedup-reset teardown guard", () => {
  it("every test file that uses warnOnce without a full vi.mock also invokes __resetWarnDedupForTests()", () => {
    const thisFile = path.resolve(__filename);

    const files = collectTestFiles(SERVER_ROOT).filter(
    ...
    for (const filePath of files) {
      ...
      if (mentionsWarnOnce && !hasFullMock && !hasResetCall) {
        violators.push(path.relative(SERVER_ROOT, filePath));
      }
    }
