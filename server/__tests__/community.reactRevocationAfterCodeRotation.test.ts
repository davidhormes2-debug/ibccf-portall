...
// portal-auth.ts reference in the POST /posts/:id/react path. These Pick<>
// declarations ensure that if any of those column names are renamed in
...
// ============================================================================
// Session revocation after access-code rotation — POST /posts/:id/react
//
...
// POST /api/community/posts/:id/react enforces this via
...
const MOCK_PARTICIPANT = {
  id: 5,
  caseId: "case-react-rotation-test",
  anonymousHandle: "Member #RCTXX",
  postCount: "0",
};

// A mock post used by the like-count update path.
const MOCK_POST = { id: 42, likeCount: "0" };

// A mock reaction returned after a successful insert.
const MOCK_REACTION = { id: 1, postId: MOCK_POST.id, participantId: MOCK_PARTICIPANT.id, reactionType: "like" };
      ...
      from: (table: any) => ({
        where: async (_pred: any) => {
          ...
          if (name === "community_reactions") {
            // No prior reaction — allow insert.
            return [];
          }
    ...
    insert: (table: any) => ({
      values: (data: any) => {
        ...
        const builder = {
          ...
          returning: async () => {
            if (name === "community_reactions") return [{ ...MOCK_REACTION, ...data }];
            return [];
          },
        };
        return builder;
...
const CASE_ID = "case-react-rotation-test";
const ORIGINAL_CODE = "ORIGINAL-CODE";
const NEW_CODE = "ROTATED-CODE";
const POST_ID = MOCK_POST.id;
...
describe(
  "POST /api/community/posts/:id/react — session revocation after access-code rotation",
  () => {
    it(
      ...
      async () => {
        ...
        const res = await request(app)
          .post(`/api/community/posts/${POST_ID}/react`)
